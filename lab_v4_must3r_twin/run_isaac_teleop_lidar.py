...
_uarg = next((a for a in args if a.endswith((".usd", ".usdz", ".usda"))), None)
if _uarg:
    USD = _uarg if Path(_uarg).is_absolute() else str(HERE / _uarg)
else:                                    # auto-deteta a cena da pasta (qualquer *_robot_scene.usdz)
    _c = HERE / "lab_v4_robot_scene.usdz"
    if not _c.exists():
        _opts = sorted(HERE.glob("*_robot_scene.usdz")) or sorted(HERE.glob("*.usdz"))
        _c = _opts[0] if _opts else _c
    USD = str(_c)
...
def main():
    ...
    # spawn: --spawn X Y (override) | --goal (usa o 2º ponto livre) | senão 'start' do nav_path
    sz = float(args[args.index("--spawn-z") + 1]) if "--spawn-z" in args else 0.20
    spawn = np.array([0.0, 0.0, sz])
    if "--spawn" in args:
        i = args.index("--spawn"); spawn = np.array([float(args[i + 1]), float(args[i + 2]), sz])
    elif (HERE / "nav_path.json").exists():
        import json
        nav = json.loads((HERE / "nav_path.json").read_text())
        s = nav.get("goal" if "--goal" in args else "start")
        if s:
            spawn = np.array([s[0], s[1], sz])
    log(f"spawn do robô: {spawn.round(2)}  (muda com --spawn X Y  [--spawn-z Z] [--goal])")
